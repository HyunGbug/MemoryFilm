/*
 * package com.film.www.config.Security;
 * 
 ...
 * 
 * import com.film.www.config.Jwt.JwtAuthenticationFilter;
 * 
 * @Configuration
 * 
 ...
 * throws Exception { http.cors(cors -> cors.disable()) // CORS 설정을 비활성화합니다.
 ...
 * .requestMatchers("/home_login_X","about","/signup").permitAll() // 로그인,회원가입
 * 페이지 및 리소스는 누구나 접근 가능 .requestMatchers("/WEB-INF/views/login/**").permitAll()
 * // WEB-INF 내부의 로그인 페이지에 대한 모든 접근 허용
 ...
 * .requestMatchers("/doctor/**", "/api/doctor/**",
 * "/dicom/**").hasRole("DOCTOR") .requestMatchers("/nurse/**").hasRole("N") //
 * 간호사만 접근 허용 .requestMatchers("/headNurse/**").hasRole("H") // 수간호사만 접근 허용
